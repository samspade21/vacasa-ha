...
class TestApiResponseParsing:
    ...
    @pytest.mark.asyncio
    async def test_get_home_info_combines_multiple_endpoints(self, api_client):
        """Home info aggregation merges summary, activity, latest, and legacy data."""
        unit_id = "122508"
        ...
        responses = {
            ("GET", f"/homes/{unit_id}"): {
                "homeId": unit_id,
                "name": "Mountain View Getaway",
                "active": True,
            },
            ("GET", f"/homes/{unit_id}/activity"): {
                "lastInspectionDate": "2025-11-09T17:00:00Z",
                "nextCleanScheduled": "2025-11-17",
                "averageGuestCleanScore": 5.0,
            },
            ("GET", f"/homes/{unit_id}/inspections/latest"): {
                "inspectionId": "abcd-1234",
                "date": "2025-11-09T17:22:00Z",
                "totalItemsInspected": 35,
            },
            ("GET", f"/owners/{api_client._owner_id}/units/{unit_id}/home-info"): {
                "data": {
                    "attributes": {
                        "homeStatus": "Ready",
                        "lastCleanDate": "2025-11-10",
                    }
                }
            },
        }
        ...
        async def fake_request(method, path, **_kwargs):
            ...
            if key not in responses:
                raise AssertionError(f"Unexpected request for {method} {path}")
            return responses[key]

        with patch.object(api_client, "_request", side_effect=fake_request):
            info = await api_client.get_home_info(unit_id)

        assert info["summary"]["name"] == "Mountain View Getaway"
        assert info["latestInspection"]["inspectionId"] == "abcd-1234"
        assert info["legacy"]["attributes"]["lastCleanDate"] == "2025-11-10"

        attrs = info["attributes"]
        assert attrs["homeId"] == unit_id
        assert attrs["nextCleanDate"] == "2025-11-17"
        assert attrs["cleanScore"] == 5.0
        assert attrs["homeStatus"] == "Ready"
        assert attrs["lastCleanDate"] == "2025-11-10"

    @pytest.mark.asyncio
    async def test_get_home_info_falls_back_to_legacy(self, api_client):
        """Legacy payloads populate attributes when newer endpoints fail."""
        unit_id = "122508"
        api_client._token = "token"
        api_client._token_expiry = datetime.now(timezone.utc) + timedelta(hours=1)
        api_client._owner_id = "owner123"
        ...
        async def fake_request(method, path, **_kwargs):
            if path.startswith(f"/homes/{unit_id}"):
                raise ApiError("not found")
            if path == f"/owners/{api_client._owner_id}/units/{unit_id}/home-info":
                return {
                    "data": {
                        "attributes": {
                            "status": "Legacy Clean",
        ...
        assert info["latestInspection"] == {}
        attrs = info["attributes"]
        assert attrs["status"] == "Legacy Clean"
        assert attrs["homeStatus"] == "Legacy Clean"
        assert attrs["lastInspectionDate"] == "2025-11-01"
